...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Zuo " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {172.104.61.247, 172.104.61.247 },{139.162.55.103 ,139.162.55.103 }
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Zuo is probably already running."), addrBind.ToString().c_str());
        else
